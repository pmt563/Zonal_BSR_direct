...
class CANDriver:
    ...
    def _configure_can_channels(self):
        ...
        ret = self.canDLL.ZCAN_SetDbitBaud(self.m_dev, 0, 1000000)
        if ret != STATUS_OK:
            print("Set CAN0 dbit:1M failed!")
            return False
        print("Set CAN0 dbit:1M OK!")

        # Configure CAN1
        ret = self.canDLL.ZCAN_SetAbitBaud(self.m_dev, 1, 500000)
        if ret != STATUS_OK:
            print("Set CAN1 abit:500K failed!")
            return False
        print("Set CAN1 abit:500K OK!")
        
        ret = self.canDLL.ZCAN_SetDbitBaud(self.m_dev, 1, 1000000)
        if ret != STATUS_OK:
            print("Set CAN1 dbit:1M failed!")
            return False
        print("Set CAN1 dbit:1M OK!")
    ...
    def _prepare_airbag_command_frame(self):
        ...
        self.frame = (ZCAN_Transmit_Data * 1)()
        self.frame[0].transmit_type = 0      # Normal transmission
        self.frame[0].frame.eff = 0          # Standard frame format
        self.frame[0].frame.rtr = 0          # Data frame
        #self.frame[0].frame.brs = 1          # Enable bit rate switching
        self.frame[0].frame.can_id = CAN_ID_AIRBAG_CMD  # 0x150
        self.frame[0].frame.can_dlc = 3
        #self.frame[0].frame.len = 3          # Data length = 3 bytes
        
        # Initial payload
        self.frame[0].frame.data[0] = 128    # Default: passenger airbag deactivate
        self.frame[0].frame.data[1] = 0      # Alive counter
        self.frame[0].frame.data[2] = 0      # Checksum
    ...
    def send_airbag_command(self, enable_airbag):
        ...
        self.frame[0].frame.data[0] = 129 if enable_airbag else 128
        ...
        self.frame[0].frame.data[1] = self.aliveCounter
        
        # Calculate and set checksum
        checksum = cal_check_sum(self.frame[0].frame.data)
        self.frame[0].frame.data[2] = checksum
        
        # Send frame
        # result = self.canDLL.ZCAN_TransmitFD(self.dev_ch1, byref(self.frame), 1)
        result = self.canDLL.ZCAN_Transmit(self.dev_ch1, self.frame, 1)
        data_sent = [hex(self.frame[0].frame.data[i]) for i in range(self.frame[0].frame.can_dlc)]
    ...
    def receive_airbag_status(self):
        ...
        """
        #print('Receive Loop')
        ret = self.canDLL.ZCAN_GetReceiveNum(self.dev_ch1, TYPE_CAN)
        if ret <= 0:
            return False, None, None
            
        rcv_msgs = (ZCAN_Receive_Data * ret)()
        num = self.canDLL.ZCAN_Receive(self.dev_ch1, byref(rcv_msgs), ret, 100)
        
        if num <= 0:
            return False, None, None
            
        for i in range(num):
            data = [rcv_msgs[i].frame.data[j] for j in range(rcv_msgs[i].frame.can_dlc)]
